...
def change_collision_behaviour_client():
    ...
    try:
        ...
        lower_torque_thresholds_acceleration = [40, 40, 40, 40, 40, 40, 40]
        upper_torque_thresholds_acceleration = [40, 40, 40, 40, 40, 40, 40]
        lower_torque_thresholds_nominal = [40, 40, 40, 40, 40, 40, 40]
        upper_torque_thresholds_nominal = [40, 40, 40, 40, 40, 40, 40]
        lower_force_thresholds_acceleration = [40, 40, 40, 40, 40, 40]
        upper_force_thresholds_acceleration = [40, 40, 40, 40, 40, 40]
        lower_force_thresholds_nominal = [40, 40, 40, 40, 40, 40]
        upper_force_thresholds_nominal = [40, 40, 40, 40, 40, 40]
...
def recovery_client():
    client = actionlib.SimpleActionClient('/franka_control/error_recovery', franka_control.msg.ErrorRecoveryAction)
    print('waiting')
    client.wait_for_server()
    print('waiting finished')
    goal = franka_control.msg.ErrorRecoveryGoal()
...
class MoveitObjectHandler(object):
    ...
    def add_box_object(self, id_name, pose, size = (1, 1, 1), rotation = (0, 0, 0), frame='/world'):
        ...
        q_object = [pose.pose.orientation.x, pose.pose.orientation.y, pose.pose.orientation.z, pose.pose.orientation.w]
    ...
    def add_cylinder_object(self, id_name, pose, height, radius, rotation = (0, 0, 0), frame='/world'):
	    ...
	    #Add object description in scene
        self.clear_octomap() #need to sleep before adding part!! https://answers.ros.org/question/209030/moveit-planningsceneinterface-addbox-not-showing-in-rviz/
        self.planning_scene_interface.add_cylinder(id_name, object_pose, height, radius)
        self.wait_for_planning_scene_object(id_name)

    def add_plane_object(self, id_name, pose, normal = (0, 0, 1), offset=0, rotation = (0, 0, 0), frame='/world'):
        assert not id_name in self.scene_objects, 'Object with the same name already exists!' 
        ...
        # add object description in scene
        self.clear_octomap()
        self.planning_scene_interface.add_plane(id_name, pose, normal, offset)
        self.wait_for_planning_scene_object(id_name)
    ...
    def get_object_positions(self, object_name):
        ...
        '''
        print("haha")
        print(self.planning_scene_interface.get_object_poses(object_name))
