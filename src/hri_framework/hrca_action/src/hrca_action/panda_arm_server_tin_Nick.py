...
class ArmServer(object):
    ...
    def arm_action(self, arm, action, pose1, pose2, object_name, hover_distance=0.1):
        ...
        def move_to_pick_hover_pose(userdata):
            ...
            pick_object_pose.pose.position.x -= 0.01
            print(pick_object_pose)
            commander.set_pose_target(pick_object_pose)
            ...
            # plan = self._arm.plan_to_pose(pick_object_pose)
        ...
        def grasp_and_hover(userdata):
            ...
            if arm.simulation:
                ...
                grasp_success = self._arm.grasp(width=0.01, force = 20)
        ...
        @smach.cb_interface(outcomes=['done', 'aborted', 'failed_place', 'failed'], input_keys=['place_object_pose','hover_distance', 'object_name'])
        ...
        if goal.action == "pick_and_place":
            ...
            if move_success:
                if self._arm.simulation:
                    ...
                    success = self._arm.grasp(width = 0.01, force=20)
