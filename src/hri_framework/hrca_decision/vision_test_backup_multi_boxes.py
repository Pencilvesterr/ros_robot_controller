...
def box1():
    ...
    coco_place = create_pose_stamped(create_pose(row1_x, -0.1, 0.05, -1, 0, 0, 0), "panda_link0") 
...
def box2():
    ...
    # DEFINE PACKING OBJECTS 
    [muesli_pose, muesli_pick, muesli_place] = get_object_info(item_size, pub, "MUESLI", "first", row1_x)
    [vita_pose, vita_pick, vita_place] = get_object_info(item_size, pub, "VITA", "second", 0.55)
    [vapo_pose, vapo_pick, vapo_place] = get_object_info(item_size, pub, "VAPO2", "third", row1_x)
    
        # ADD COLLISION OBJECTS
    moh.add_box_object("muesli", pose=muesli_pose, size=item_size["MUESLI"], rotation=(0,0,0), frame="panda_link0")
    moh.add_box_object("vita", pose=vita_pose, size=item_size["VITA"], rotation=(0,0,0), frame="panda_link0")
    ...
    result = call_server("pick_and_place", vapo_pick, vapo_place, "vapo2")
    moh.remove_world_object("vapo2")
