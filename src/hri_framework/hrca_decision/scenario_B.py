...
########################################################################################################################################################
#                                                        SCENARIO B - ERROR AND EXPLANATION                                                            #
########################################################################################################################################################  

# THINGS TO ADD TO OTHER SCENARIOS
# - WAIT FOR HUMAN TURN 
# - ERROR RECOVERY ONCE IT WORKS 
...
def get_object_info(item_dict, pub, item_name, place_pose, place_x, stack = "no_stack"):
    ...
    if (stack == "no_stack"):
        ...
        if place_pose == "highsecond":
            place_height = 2*pick_height
        else:
...
def test():
    ...
    print("RESULT is: " + str(result.success))
    error_catch(result.success)
...
def box1():
    ...
    p1 = subprocess.Popen(["mpg321", "--gain", "20", PREFIX_PATH + "scanning_wait.mp3"], shell=False, stdout=devnull)
    ...
    p0 = subprocess.Popen(["mpg321", "--gain", "20", PREFIX_PATH + "human_prompt.mp3"], shell=False, stdout=subprocess.PIPE, stderr=devnull)
    ...
    p1 = subprocess.Popen(["mpg321", "--gain", "20", PREFIX_PATH + "box_reminder.mp3"], shell=False, stdout=subprocess.PIPE, stderr=devnull)
...
def box2():
    ...
    p1 = subprocess.Popen(["mpg321", "--gain", "20", PREFIX_PATH + "I.mp3"], shell=False, stdout=subprocess.PIPE, stderr=devnull)
    time.sleep(0.5)
    p2 = subprocess.Popen(["mpg321", "--gain", "20", PREFIX_PATH + "mistake.mp3"], shell=False, stdout=subprocess.PIPE, stderr=devnull)
    while p2.poll() is None:
        time.sleep(0.1)
...
def box4():
    ...
    [muesli2_pose, muesli2_pick, muesli2_place] = get_object_info(item_size, pub, "MUESLI2", "highsecond", row1_x, "stack")
